...
<?php
	...
	$passwd = $_POST['passwd'];
	//$page_name=$_REQUEST
    $chk_dup_temp="select * from manager_info where username='".$username."'";
    $dup_result1= mysqli_query($dbhandle,$chk_dup_temp);
	$total_rows=mysqli_num_rows($dup_result1); 
	if($total_rows==0)
	    {
	       include("incorrect-manager-idlogin.php");
          //echo "hghjghj";		   
	    }
	else{
           while ($duprow = mysqli_fetch_array($dup_result1,MYSQLI_NUM))
                {
			      if(strcmp($passwd,$duprow{1})!=0)
					  {
				        include("incorrect-manager-passwd-login.php");  
				      }
				  else{ 	  
                        $name=$duprow{4};
			            $email=$duprow{3};
    ...
    <div class="main_content">
                    ...
                    <div class="menu">
                    <ul>
                    <li><a class="current" href="#">Manager Home<i class="material-icons">home</i></a></li>
