...
<head>
	...
	$passwd = $_POST['passwd'];
	//$page_name=$_REQUEST
    $chk_dup_temp="select * from employee_info where emailid='".$emailid."'";
    $dup_result1= mysqli_query($dbhandle,$chk_dup_temp);
	$total_rows=mysqli_num_rows($dup_result1); 
	if($total_rows==0)
	    {
	       include("incorrect_idlogin.php");  
	    }
	else{
           while ($duprow = mysqli_fetch_array($dup_result1,MYSQLI_NUM))
                {
			      if(strcmp($passwd,$duprow{3})!=0)
					  {
				        include("incorrect_passwd_login.php");  
				      }
				  else{ 	  
                        $name=$duprow{2};
			            $eid=$duprow{1};
